...
namespace CaseOpgave.Codes
{
    ...
    internal sealed class CurrentUserSession : Products
    {
        ...
        public void SetUserSession()
        {
            ...
            FilePath = @"C:\Users\mathi\Desktop\CaseOpgave\Movie product data\CurrentUserSession.txt";
        ...
        public List<ProductModel> CostumerView(string getUserId)
        {
            ...
            {
                //add uotput userinput til matchProductId
                ...
                {
                    //matching product id med userProductId
                    foreach (var productInfo in Product)
                    {
                        if (productInfo.Id.Equals(userInfo.ProductId))
                            displayCostumerSession.Add(Product.FirstOrDefault(x => productInfo.Id == userInfo.ProductId));
                    }
                    //displayCostumerSession = Product.Where(x => x.ProductId.Contains(item.ProductId)).ToList();
                    //displayCostumerSession.AddRange(Product.Where(a => a.ProductId == userInfo.ProductId));
                }
            };
